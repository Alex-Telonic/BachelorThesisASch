...
,,U.S. Commercial Banks: Trends and Cycles``\\\vspace{0.5cm}
...
\end{itemize}
For further context awareness, it is important to mention following other structural events that affected the U.S. commercial banks landscape considerably:
...
\item Basel 1 in 1988, Basel 2 in 2004, Basel 3 in 2010 - Capital and liquidity regulations to improve banking sectors' stability.
\item Over our data time-frame the banking sector experienced a wide-spread adoption of financial innovations. The main ones being interest rate derivatives, asset securitization and adjustable rate mortgages.  
\end{itemize}
...
For most methods we transform the data with the natural logarithm to focus on relative changes. In graphs we will indicate this transformation with proper labels. Furthermore, we apply the recognized Hodrick-Prescott Filter with the recommended parameter of 1600 for quarterly time-series to de-trend out data. Seasonal effects will not be removed. The resulting cyclical graphs show the relative cyclical variations of the underlying variable. For correlations and autocorrelations, we use the linear Pearsons' correlation coefficient. To determine significance we compute the 2-tailed p-value. Significance is then determined according to following levels:
\begin{enumerate}
\item ***: <0.01
\item **: <0.05
\item *: <0.1
...
This section will guide us through the distribution of financial components held by the U.S. commercial banking sector as a whole. We will see what types and amounts of financial instruments banks are holding and how these positions evolved over time. 
...
Diving into more detailed analysis of the individual balance sheet accounts, Figure \ref{fig:positions} gives us the detrended development of each individual position for both the asset side(left column) and liabilities side(right column) of a balance sheet. As a complement, Figure \ref{fig:share_both} shows the share of each position in relation to the total assets. \\
...
Reasons for the trends we have just documented are not absolute clear. However, geographic deregulation and other regulation reforms such as the repeal of the Glass-Steagall act in 1999 did support the increasing inequality. In addition, larger banks are more likely to be bailed out. This puts them in an easier position to finance themselves and creates the perverse consequence of a moral hazard. A bank with high likelihood to be bailed out takes on too much risk (\cite{FarhiTirole2012}). The severe consequences of this problematic are clear since the financial crisis in 2008. Authorities responded to this issue by setting additional capital requirements on larger banks with frameworks such as Basel 1,2,3. 
...
\includegraphics[scale=0.35]{graphs/DescriptiveStats/BanksbyAssetsCategory_ScatterAverageLeveragevsAggregateAssetsCat_7613.png}
...
In general, this article gives a broad overview over the U.S. commercial bank landscape and key important factors that should be considered. We find interesting trends and cycles on an aggregate level as well as for different bank sizes. Many points discussed are open for interpretation and future work should go into more detail about the approached topics. For instance, it would be interesting to find an optimal way to categorize us commercial banks. The literature seems to have found no coherent way of categorization. These categories would obviously be of key importance to regulators. Furthermore, as seen from the large amount of literature about leverage there is wide variety of possibilities to explore it in greater depths. One might consider the relationship of leverage not just with assets, but GDP and other variables.\\
Moreover, commercial banks are just a part of the financial intermediaries an existing economies have. The so called shadow banking sector does play a major role in today's financial industry and hold a significant share of total assets. It also were investment banks which had to bear the major impacts of the financial crisis in 2007, not commercial banks.\\   
Lastly, another factor we have not considered is that according to \cite{Kalemli-OzcanBentSorensenSevcanYesiltas2011} a big fraction of assets, especially for large commercial banks, are off balance sheet items. 
